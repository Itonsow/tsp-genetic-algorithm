...
// Plota uma rota (pontos e arestas) e salva como SVG
inline bool plotRota(const InstanciaTSP &tsp, const Rota &rota, const std::string &filename,
                     const std::string &title = "")
{
    const auto &ptos = tsp.getPoints();

    // Extrai coordenadas
    std::vector<double> x, y;
    for (int idx : rota)
    {
        x.push_back(ptos[idx].x);
        y.push_back(ptos[idx].y);
    }
    // Adiciona a primeira ponto novamente para fechar o loop
    x.push_back(ptos[rota[0]].x);
    y.push_back(ptos[rota[0]].y);

    // Todas as posições das pontos para dispersão
    std::vector<double> all_x, all_y;
    for (const auto &pto : ptos)
    {
        all_x.push_back(pto.x);
        all_y.push_back(pto.y);
    }
    ...
    plot.line(x, y, "#0066cc", 2.0, "Rota");

    // Desenha pontos como pontos dispersos
    plot.scatter(all_x, all_y, "#ff6600", 6.0, "Pontos");

    return plot.write(filename);
}

// Plota uma rota com informações da geração
inline bool plotRotaEpoca(const InstanciaTSP &tsp, const Individual &melhor, int epocas,
                          const GAConfig & /* config */, const std::string &filename)
{
    ...
    return plotRota(tsp, melhor.rota, filename, title.str());
}

// Plota curva de convergência (melhor aptidão ao longo das gerações)
inline bool plotaConvergencia(const std::vector<double> &melhor_por_epoca,
...
inline bool salvarMetricasCSV(const std::string &filename,
...
inline bool salvarRotaParaFile(const std::string &filename, const Rota &rota, double fitness)
{
    std::ofstream file(filename);
    if (!file.is_open())
        return false;
    ...
    file << " -> " << rota[0] << " (retorna para o inicio)\n";
...
inline void salvarEpocaFrame(const InstanciaTSP &tsp, const Individual &melhor, int epocas,
                             const GAConfig &config, const std::string &framesdir,
                             int frame_interval = 1)
{
    if (epocas % frame_interval != 0 && epocas != 0)
        return;

    std::ostringstream filename;
    filename << framesdir << "/epocas_" << std::setfill('0') << std::setw(4) << epocas << ".svg";

    plotRotaEpoca(tsp, melhor, epocas, config, filename.str());
}
